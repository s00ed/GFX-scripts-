/**
 * SpotColorKeyOrder_TEXT_LIST_FULL_SPEED.jsx
 * ─────────────────────────────────────────────
 * FULL SPEED VERSION
 *
 ...
 * IMPORTANT:
 * - NO GROUP.
 * - TextFrames are created directly inside layer.
 * - Fast collection.
 * - Ignores old "Color Key" layer.
 * - Does NOT scan every character in text.
 * - Fast mathematical center alignment.
...
(function () {
    ...
    var CFG = {
        ...
        // speed mode:
        // true = faster, scans object-level colors only.
        // false = slower, can detect mixed character colors inside text.
        fastTextScan: true,

        // true = ignore old output layer while collecting colors.
        ignoreOutputLayer: true,

        // true = clear old output before creating new one.
        clearOldOutput: true
    };
    ...
    function getReferenceCenter() {
        if (DOC.selection && DOC.selection.length > 0) {
            try {
                return getCenter(DOC.selection[0].visibleBounds);
            } catch (e1) {
                try {
                    return getCenter(DOC.selection[0].geometricBounds);
                } catch (e2) {}
            }
        }
    ...
    function showDashboard(spots) {
        ...
        optRow.alignment = "center";
        optRow.spacing = 30;

        var cbAlign = optRow.add("checkbox", undefined, "align to selected object center");
        cbAlign.value = CFG.alignDefault;

        var cbOP = optRow.add("checkbox", undefined, "overprint");
        ...
        btnCreate.onClick = function () {
            ...
            createTextList(entries, cbAlign.value, cbOP.value);
        };
    ...
    // =========================================================
    // CREATE TEXT LIST - FAST - NO GROUP
    // =========================================================

    function createTextList(entries, doAlign, doOverprint) {
        ...
        var startX = doAlign ? ref.x : ab[0] + CFG.offsetFromArtboardLeft;
        var startY = doAlign ? ref.y : ab[1] - CFG.offsetFromArtboardTop;
        ...
        for (var i = 0; i < entries.length; i++) {
            ...
            createdTexts.push(tf);
        }

        if (doAlign) {
            batchCenterItemsToPoint(createdTexts, ref.x, ref.y);
        }

        try {
            DOC.selection = null;

            for (var s = 0; s < createdTexts.length; s++) {
                createdTexts[s].selected = true;
            }
        } catch (eSel) {}

        app.redraw();

        alert("Done. Full speed version. No group.");
    }
    ...
    function batchCenterItemsToPoint(items, cx, cy) {
        for (var i = 0; i < items.length; i++) {
            try {
                var item = items[i];
